...
- [Overview](#overview)
  - [Why using Xpert?](#why-using-xpert)
    - [Features](#features)
...
Xpert (the Persian word for "border guard" - pronounced /mærz'ban/) is a proxy management tool that provides a simple and easy-to-use user interface for managing hundreds of proxy accounts powered by [Xray-core](https://github.com/XTLS/Xray-core) and built using Python and Reactjs.

## Why using Xpert?

Xpert is user-friendly, feature-rich and reliable. It lets you to create different proxies for your users without any complicated configuration. Using its built-in web UI, you are able to monitor, modify and limit users.

### Features

- Built-in **Web UI**
- Fully **REST API** backend
- [**Multiple Nodes**](#xpert-node) support (for infrastructure distribution & scalability)
...
Run the following command to install Xpert with SQLite database:

```bash
sudo bash -c "$(curl -sL https://github.com/Gozargah/Xpert-scripts/raw/master/xpert.sh)" @ install
```

Run the following command to install Xpert with MySQL database:

```bash
sudo bash -c "$(curl -sL https://github.com/Gozargah/Xpert-scripts/raw/master/xpert.sh)" @ install --database mysql
```

Run the following command to install Xpert with MariaDB database:
```bash
...
- The Xpert files will be located at `/opt/xpert`
- The configuration file can be found at `/opt/xpert/.env` (refer to [configurations](#configuration) section to see variables)
- The data files will be placed at `/var/lib/xpert`
- For security reasons, the Xpert dashboard is not accessible via IP address. Therefore, you must [obtain SSL certificate](https://gozargah.github.io/xpert/en/examples/issue-ssl-certificate) and access your Xpert dashboard by opening a web browser and navigating to `https://YOUR_DOMAIN:8000/dashboard/` (replace YOUR_DOMAIN with your actual domain)
- You can also use SSH port forwarding to access the Xpert dashboard locally without a domain. Replace `user@serverip` with your actual SSH username and server IP and Run the command below:

```bash
ssh -L 8000:localhost:8000 user@serverip
```

Finally, you can enter the following link in your browser to access your Xpert dashboard:
...
Next, you need to create a sudo admin for logging into the Xpert dashboard by the following command
...
To see the help message of the Xpert script, run the following command
...
If you want to use `xpert-cli`, you should link it to a file in your `$PATH`, make it executable, and install the auto-completion:
...
To launch with linux systemctl (copy xpert.service file to `/var/lib/xpert/xpert.service`)
...
The [Xpert Documentation](https://gozargah.github.io/xpert) provides all the essential guides to get you started, available in three languages: Farsi, English, and Russian. This documentation requires significant effort to cover all aspects of the project comprehensively. We welcome and appreciate your contributions to help us improve it. You can contribute on this [GitHub repository](https://github.com/Gozargah/gozargah.github.io).


# API

Xpert provides a REST API that enables developers to interact with Xpert services programmatically. To view the API documentation in Swagger UI or ReDoc, set the configuration variable `DOCS=True` and navigate to the `/docs` and `/redoc`.


# Backup

It's always a good idea to backup your Xpert files regularly to prevent data loss in case of system failures or accidental deletion. Here are the steps to backup Xpert:

1. By default, all Xpert important files are saved in `/var/lib/xpert` (Docker versions). Copy the entire `/var/lib/xpert` directory to a backup location of your choice, such as an external hard drive or cloud storage.
2. Additionally, make sure to backup your env file, which contains your configuration variables, and also, your Xray config file. If you installed Xpert using xpert-scripts (recommended installation approach), the env and other configurations should be inside `/opt/xpert/` directory.

Xpert's backup service efficiently zips all necessary files and sends them to your specified Telegram bot. It supports SQLite, MySQL, and MariaDB databases. One of its key features is automation, allowing you to schedule backups every hour. There are no limitations concerning Telegram's upload limits for bots; if a file exceeds the limit, it will be split and sent in multiple parts. Additionally, you can initiate an immediate backup at any time.

Install the Latest Version of Xpert Command:
```bash
...
By following these steps, you can ensure that you have a backup of all your Xpert files and data, as well as your configuration variables and Xray configuration, in case you need to restore them in the future. Remember to update your backups regularly to keep them up-to-date.

# Telegram Bot

Xpert comes with an integrated Telegram bot that can handle server management, user creation and removal, and send notifications. This bot can be easily enabled by following a few simple steps, and it provides a convenient way to interact with Xpert without having to log in to the server every time.
...
Xpert comes with an integrated CLI named `xpert-cli` which allows administrators to have direct interaction with it.

If you've installed Xpert using easy install script, you can access the cli commands by running

```bash
xpert cli [OPTIONS] COMMAND [ARGS]...
```

For more information, You can read [Xpert CLI's documentation](./cli/README.md).

# Xpert Node

The Xpert project introduces the [Xpert-node](https://github.com/gozargah/xpert-node), which revolutionizes infrastructure distribution. With Xpert-node, you can distribute your infrastructure across multiple locations, unlocking benefits such as redundancy, high availability, scalability, flexibility. Xpert-node empowers users to connect to different servers, offering them the flexibility to choose and connect to multiple servers instead of being limited to only one server.
For more detailed information and installation instructions, please refer to the [Xpert-node official documentation](https://github.com/gozargah/xpert-node)

# Webhook notifications

You can set a webhook address and Xpert will send the notifications to that address.
...
Example request sent from Xpert:
...
If you found Xpert useful and would like to support its development, you can make a donation in one of the following crypto networks:
...
We ❤️‍🔥 contributors! If you'd like to contribute, please check out our [Contributing Guidelines](CONTRIBUTING.md) and feel free to submit a pull request or open an issue. We also welcome you to join our [Telegram](https://t.me/gozargah_xpert) group for either support or contributing guidance.

Check [open issues](https://github.com/gozargah/xpert/issues) to help the progress of this project.

<p align="center">
Thanks to the all contributors who have helped improve Xpert:
</p>
<p align="center">
