# Contribute to Xpert
Thanks for considering contributing to Xpert!

## Questions

Please don't ask your questions in issues. Instead, use one of the following ways to ask:
- Ask on our telegram group: [@Gozargah_Xpert](https://t.me/gozargah_xpert)
- Ask on our [GitHub Discussions](https://github.com/gozargah/xpert/discussions) for long term discussion or larger questions.
...
- Also tell the version of Xpert, Xray and docker (if you use docker) you are using.
...
## Xpert CLI
Xpert CLI is built using [Typer](https://typer.tiangolo.com/), and its commands' code can be found in `cli`  directory. Its documentation is generated using [Typer CLI](https://typer.tiangolo.com/typer-cli/) which can be re-generated by navigating to project's root directory and running the following command (`typer-cli` package needs to be installed first):

```bash
$ PYTHONPATH=$(pwd) typer xpert-cli.py utils docs --name "" --output ./cli/README.md
```
